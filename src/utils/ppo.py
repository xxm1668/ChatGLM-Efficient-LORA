...
class PPOTrainerForChatGLM(PPOTrainer, PeftTrainer):
    ...
    def __init__(
        ...
        self.data_collator = self.accelerator.prepare(
            kwargs["data_collator"])  # override the data collator of PPOTrainer
    ...
    def ppo_train(self, max_target_length: int) -> None:
        ...
        for step in tqdm(range(max_steps), disable=not self.is_world_process_zero()):
            ...
            for _ in range(self.config.gradient_accumulation_steps):
                ...
                # print(queries[0])
                # print(responses[0])
                # print(self.prepare_model_inputs(queries, responses))
                # print('-------@@@@@@@@@@@@@------')
                out_text = self.tokenizer.decode(queries[0])
                # queries2 = [torch.tensor(self.tokenizer.encode(out_text)).cuda()]
                # print(queries2)
                # print(out_text)
                # print('----------')
                # # print(responses)
                # out_text = self.tokenizer.decode(responses[0])
                # print(out_text)
                # exit(-1)
    ...
    def batched_forward_pass(
        ...
        for i in range(int(bs / fbs)):
            ...
            if self.is_distributed:  # re-generate them to adapt padded inputs
                input_kwargs["attention_mask"] = self.data_collator.get_attention_masks(input_ids,
                                                                                        device=self.current_device)
                input_kwargs["position_ids"] = self.data_collator.get_position_ids(input_ids,
                                                                                   device=self.current_device)
